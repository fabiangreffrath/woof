...
patch_t *V_CacheLumpNum(int lump, pu_tag tag)
{
    if ((unsigned)lump >= numlumps)
    {
        I_Error("R_CacheLumpNum: %i >= numlumps", lump);
    }
    ...
    spng_set_png_buffer(ctx, buf, W_LumpLength(lump));
    ...
    if (ret)
    {
        I_Printf(VB_ERROR, "R_CacheNumLump: spng_get_ihdr %s\n", spng_strerror(ret));
        goto error;
    }

    if (ihdr.color_type != SPNG_COLOR_TYPE_INDEXED || ihdr.bit_depth != 8)
    {
        I_Printf(VB_ERROR, "R_CacheNumLump: Only 8-bit paletted PNG supported");
        goto error;
    }
    ...
    {
        I_Printf(VB_ERROR, "R_CacheNumLump: spng_get_plte %s\n", spng_strerror(ret));
        goto error;
    }
    ...
    if (ret && ret != SPNG_ECHUNKAVAIL)
    {
        I_Printf(VB_ERROR, "R_CacheNumLump: spng_get_trns %s\n", spng_strerror(ret));
        goto error;
    }
    ...
    if (ret)
    {
        I_Printf(VB_ERROR, "R_CacheNumLump: spng_decode_image %s\n", spng_strerror(ret));
        goto error;
    }
