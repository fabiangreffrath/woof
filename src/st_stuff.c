...
void ST_refreshBackground(void)
{
    ...
    if (video.unscaledw != ST_WIDTH)
    {
        ...
        else
        {
            ...
            {
                int x;
                patch_t *patch = V_CacheLumpName("brdr_b", PU_CACHE);
...
void ST_loadGraphics(void)
{
  ...
  for (i=0;i<10;i++)
    {
      ...
      tallnum[i] = V_CacheLumpName(namebuf, PU_STATIC);
      M_snprintf(namebuf, sizeof(namebuf), "STYSNUM%d", i);
      shortnum[i] = V_CacheLumpName(namebuf, PU_STATIC);
    }

  // Load percent key.
  //Note: why not load STMINUS here, too?
  tallpercent = V_CacheLumpName("STTPRCNT", PU_STATIC);

  // key cards
  for (i=0;i<NUMCARDS+3;i++)  //jff 2/23/98 show both keys too
    {
      M_snprintf(namebuf, sizeof(namebuf), "STKEYS%d", i);
      keys[i] = V_CacheLumpName(namebuf, PU_STATIC);
    }

  // arms background
  armsbg = V_CacheLumpName("STARMS", PU_STATIC);
  ...
  for (i=0;i<6;i++)
    {
      ...
      // gray #
      arms[i][0] = V_CacheLumpName(namebuf, PU_STATIC);
  ...
  for (i=0; i<MAXPLAYERS; i++)
    {
      ...
      faceback[i] = V_CacheLumpName(namebuf, PU_STATIC);
    }

  // status bar background bits
  if (W_CheckNumForName("STBAR") >= 0)
  {
    sbar  = V_CacheLumpName("STBAR", PU_STATIC);
    sbarr = NULL;
  }
  else
  {
    sbar  = V_CacheLumpName("STMBARL", PU_STATIC);
    sbarr = V_CacheLumpName("STMBARR", PU_STATIC);
  }
      ...
      for (j=0;j<ST_NUMSTRAIGHTFACES;j++)
        {
          ...
          faces[facenum++] = V_CacheLumpName(namebuf, PU_STATIC);
        }
  ...
  faces[facenum++] = V_CacheLumpName("STFGOD0", PU_STATIC);
  faces[facenum++] = V_CacheLumpName("STFDEAD0", PU_STATIC);
...
static int StatusBarBufferHeight(void)
{
  ...
  patch_t *const patch = V_CacheLumpName("brdr_b", PU_CACHE);
