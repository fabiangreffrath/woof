...
void R_DrawBorder (int x, int y, int w, int h)
{
  ...
  patch = V_CacheLumpName("brdr_t", PU_CACHE);
  for (i = 0; i < w; i += 8)
    V_DrawPatch(x + i - video.deltaw, y - 8, patch);

  patch = V_CacheLumpName("brdr_b", PU_CACHE);
  for (i = 0; i < w; i += 8)
    V_DrawPatch(x + i - video.deltaw, y + h, patch);

  patch = V_CacheLumpName("brdr_l", PU_CACHE);
  for (j = 0; j < h; j += 8)
    V_DrawPatch(x - 8 - video.deltaw, y + j, patch);

  patch = V_CacheLumpName("brdr_r", PU_CACHE);
  for (j = 0; j < h; j += 8)
  ...
  V_DrawPatch(x - 8 - video.deltaw,
              y - 8,
              V_CacheLumpName("brdr_tl", PU_CACHE));
    
  V_DrawPatch(x + w - video.deltaw,
              y - 8,
              V_CacheLumpName("brdr_tr", PU_CACHE));
    
  V_DrawPatch(x - 8 - video.deltaw,
              y + h,
              V_CacheLumpName("brdr_bl", PU_CACHE));
    
  V_DrawPatch(x + w - video.deltaw,
              y + h,
              V_CacheLumpName("brdr_br", PU_CACHE));
}
