...
static boolean CheckDeathUseAction(void)
{
    boolean result = ((players[consoleplayer].playerstate & PST_DEAD)
                       && death_use_action != death_use_default);
    if (result)
    {
        activate_death_use_action = true;
    }
    return result;
}
