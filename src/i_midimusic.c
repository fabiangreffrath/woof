...
{
    M_BindInt("midi_complevel", &midi_complevel, COMP_STANDARD, 0, COMP_NUM - 1,
        "Native MIDI compatibility level (0 = Vanilla, 1 = Standard, 2 = Full)");
    M_BindInt("midi_reset_type", &midi_reset_type, RESET_TYPE_GM, 0, RESET_NUM - 1,
        "Reset type for native MIDI (0 = No SysEx, 1 = GM, 2 = GS, 3 = XG)");
    M_BindInt("midi_reset_delay", &midi_reset_delay, -1, -1, 2000,
        "Delay after reset for native MIDI (-1 = Auto, 0 = None, 1-2000 = Milliseconds)");
    M_BindBool("midi_ctf", &midi_ctf, true,
