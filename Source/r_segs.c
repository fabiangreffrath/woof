...
void R_StoreWallRange(const int start, const int stop)
{
  ...
  topfrac = ((int64_t)centeryfrac>>4) - (((int64_t)worldtop*rw_scale)>>FRACBITS);

  bottomstep = -FixedMul (rw_scalestep,worldbottom);
  bottomfrac = ((int64_t)centeryfrac>>4) - (((int64_t)worldbottom*rw_scale)>>FRACBITS);
  ...
  if (backsector)
    {
      ...
      if (worldhigh < worldtop)
        {
          pixhigh = ((int64_t)centeryfrac>>4) - (((int64_t)worldhigh*rw_scale)>>FRACBITS);
          pixhighstep = -FixedMul (rw_scalestep,worldhigh);
        }
      if (worldlow > worldbottom)
        {
          pixlow = ((int64_t)centeryfrac>>4) - (((int64_t)worldlow*rw_scale)>>FRACBITS);
